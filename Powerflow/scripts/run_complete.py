...
Usage:
    python run_complete.py                          # Full run
...
def main():
    ...
    if not args.skip_adapter:
        run_step("Data Adapter", [python, str(SCRIPT_DIR / "Jason_data_adapter.py")])
    else:
        logger.info("\nSkipping data adapter (--skip-adapter)")

    # Step 2: Prepare power flow
    if not args.skip_prepare:
        run_step("Prepare Power Flow", [python, str(SCRIPT_DIR / "Jason_prepare_powerflow.py")])
    else:
    ...
    run_step("Run Power Flow", pf_cmd)

    # Step 4: Compare losses (only if mode includes both runs)
    if args.mode == 'both':
        run_step("Compare Losses", [python, str(SCRIPT_DIR / "Jason_compare_losses.py")])
    else:
        logger.info(f"\nSkipping loss comparison (mode={args.mode}, need 'both')")

    total_elapsed = time.time() - total_start
    logger.info(f"\n{'='*80}")
    logger.info(f"Pipeline complete! Total time: {total_elapsed:.1f}s ({total_elapsed/60:.1f} min)")
    logger.info(f"{'='*80}")
